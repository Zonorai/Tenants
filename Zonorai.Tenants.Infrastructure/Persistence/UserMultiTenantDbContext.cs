...
namespace Zonorai.Tenants.Infrastructure.Persistence
{
    public abstract class UserMultiTenantDbContext : MultiTenantDbContext
    {
        ...
        protected UserMultiTenantDbContext(ITenantInfo tenantInfo, IEventStore eventStore, IMediator mediator) : base(tenantInfo)
        {
            EventStore = eventStore;
            Mediator = mediator;
        }

        protected UserMultiTenantDbContext(ITenantInfo tenantInfo, DbContextOptions options, IEventStore eventStore, IMediator mediator) : base(tenantInfo, options)
        {
            EventStore = eventStore;
            Mediator = mediator;
        }
        ...
        public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
        {
            ...
            foreach (var @event in EventStore.Events)
            {
